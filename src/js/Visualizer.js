 ...
 * Uses a vibrant color gradient with pronounced contrast
...
function mapValueToColor(value, analysisType = '') {
    ...
    // New flashy/pronounced color gradient
    // Creates a vibrant spectrum: Deep Blue → Cyan → Green → Yellow → Orange → Red
    if (normalizedValue <= 0.0) {
        ...
        const b = Math.floor(128 * (1 - t) + 255 * t);
        return (r << 16) | (g << 8) | b;
...
function createColorLegend(maxScore, analysisType = '', topOffset = 10) {
    ...
    // Use FIXED ranges for consistency - get from centralized function
    const fixedRanges = getFixedRanges(analysisType);
    ...
    for (let i = 0; i < fixedRanges.length; i++) {
        ...
        if (analysisType === 'CELL' || analysisType === 'FACE') {
            ...
            let savedOpacity = 0.1; // Default to 10% opacity
            ...
            // Cell count display
            legendHTML += `<span id="legend-count-${analysisType.toLowerCase()}-${i}" style="font-size: 10px; margin-left: 4px; width: 30px; color: #666;">(0)</span>`;
        }
...
function getOpacityForScore(score, maxScore) {
    ...
    return 0.1; // Default if not found (10%)
}
...
function initializeOpacitySettings() {
    ...
    if (!window.legendOpacities || window.legendOpacities.length === 0) {
        ...
        window.savedLegendOpacities = opacities; // Also save to global for persistence
    }
}
...
/**
 * Get the fixed ranges for a given analysis type
 * @param {string} analysisType - Type of analysis (CELL, FACE, VERTEX, EDGE)
 * @returns {Array} Array of range objects with start, end, and label properties
 */
function getFixedRanges(analysisType) {
    if (analysisType === 'FACE') {
...
/**
 * Update cell counts in the legend
 * @param {string} analysisType - Type of analysis (CELL, FACE, VERTEX, EDGE)
 * @param {Array} scores - Array of scores to count
 */
function updateLegendCounts(analysisType, scores) {
    if (!scores || scores.length === 0) return;
    
    // Get the ranges for this analysis type - uses same function as legend
    const fixedRanges = getFixedRanges(analysisType);
    
    // Only update counts for CELL and FACE types
    if (analysisType !== 'CELL' && analysisType !== 'FACE') {
        return;
    }
    
    // Count cells in each range
    const rangeCounts = new Array(fixedRanges.length).fill(0);
    ...
    rangeCounts.forEach((count, i) => {
        const countElement = document.getElementById(`legend-count-${analysisType.toLowerCase()}-${i}`);
        if (countElement) {
...
export function createAndShowLegend(analysisType, scores, verticalOffset = 0) {
    ...
    // Update cell counts
    updateLegendCounts(analysisType, scores);
}

/**
 * Export the updateLegendCounts function for external use
 * @param {string} analysisType - Type of analysis (CELL, FACE, VERTEX, EDGE)
 * @param {Array} scores - Array of scores to count
 */
export function updateCellCounts(analysisType, scores) {
    updateLegendCounts(analysisType, scores);
}
